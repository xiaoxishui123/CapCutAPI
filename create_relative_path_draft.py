...
def create_relative_path_draft():
    ...
    print(f"🔗 音频相对路径: {audio_path}")
    print(f"🔗 视频相对路径: {video_path}")
    print(f"🔗 图片相对路径: {image_path}")
    ...
    draft_meta_info = {
        "draft_id": draft_id,
        "draft_name": f"CapCutAPI相对路径草稿_{timestamp}",
        "create_time": timestamp,
        "update_time": timestamp,
        "version": "8.7.0",
        "platform": "windows"
...
def main():
    ...
    try:
        ...
        print("2. 重启剪映专业版软件")
        print("3. 在剪映中打开草稿，媒体文件应该能正确显示在时间线上")
        
        print("\n💡 相对路径草稿的优势:")
        print("- ✅ 使用相对路径，剪映更容易识别")
        print("- ✅ 媒体文件路径格式正确")
        print("- ✅ 包含图片和音频轨道")
        print("- ✅ 媒体文件实际复制到草稿目录")
        print("- ✅ 时间线配置正确")
