...
def create_simple_draft():
    ...
    if os.path.exists(content_src):
        ...
        content_info["platform"]["device_id"] = str(uuid.uuid4()).replace("-", "")[:32]
        content_info["platform"]["hard_disk_id"] = str(uuid.uuid4()).replace("-", "")[:32]
        content_info["platform"]["mac_address"] = str(uuid.uuid4()).replace("-", "")[:32]
        
        content_info["last_modified_platform"]["os"] = "windows"
        content_info["last_modified_platform"]["os_version"] = "10.0.19045"
        content_info["last_modified_platform"]["device_id"] = str(uuid.uuid4()).replace("-", "")[:32]
        content_info["last_modified_platform"]["hard_disk_id"] = str(uuid.uuid4()).replace("-", "")[:32]
        content_info["last_modified_platform"]["mac_address"] = str(uuid.uuid4()).replace("-", "")[:32]
        
        # 更新其他必要信息
        content_info["id"] = draft_info["id"]
        content_info["create_time"] = int(datetime.now().timestamp())
        content_info["update_time"] = int(datetime.now().timestamp())
        
        with open(f"{draft_dir}/draft_content.json", "w", encoding="utf-8") as f:
            json.dump(content_info, f, ensure_ascii=False, indent=4)
        print("✅ 复制并更新文件: draft_content.json")
    else:
        print("⚠️ 文件不存在: draft_content.json")
